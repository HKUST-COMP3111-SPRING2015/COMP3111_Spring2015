...
public class AppList extends JPanel implements ActionListener {
	...
	private final String[] names = { "Time", "Appointments", "Status", "Time",
			"Appointments", "Status" };
	//private final int[] monthDays = { 31, 28, 31, 30, 31, 30, 31, 30, 31, 30, 31, 30, 31 };
	...
	private JMenuItem newItem, modifyItem, deleteItem, detailsItem;
	...
	public AppList() {
		...
		newItem = (JMenuItem) pop.add(new JMenuItem("New"));
		...
		modifyItem = (JMenuItem) pop.add(new JMenuItem("Modify"));
		...
		deleteItem = (JMenuItem) pop.add(new JMenuItem("Delete"));
	...
	private void pressResponse(MouseEvent e) {
		...
		pressRow = tableView.rowAtPoint(e.getPoint());
		pressCol = tableView.columnAtPoint(e.getPoint());
	}
	...
	private void releaseResponse(MouseEvent e) {
		...
		if ((e.getModifiers() & InputEvent.BUTTON3_MASK) != 0)
			pop.show(e.getComponent(), e.getX(), e.getY());*/
		releaseRow = tableView.rowAtPoint(e.getPoint());
		releaseCol = tableView.columnAtPoint(e.getPoint());
		if(e.getButton() == MouseEvent.BUTTON3 && pressRow == releaseRow && pressCol == releaseCol)//Right click
		{
	...
	private void modifyPopupMenu()
	{
		Object o;
		if (currentCol < 3)
			o = tableView.getModel().getValueAt(currentRow, 1);
		else
			o = tableView.getModel().getValueAt(currentRow, 4);

		if (o instanceof Appt)
		{
			newItem.setEnabled(false);
			Appt a = getSelectedAppTitle();
			if (a == null)
				return;
